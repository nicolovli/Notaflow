...
import { Alert, CircularProgress, TextField, List, ListItem, ListItemText, 
  ListItemAvatar, Avatar, Chip, Paper, Typography, IconButton, Box } from "@mui/material";
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            {selectedUsers.map(user => (
              <Chip
                key={user.id}
                avatar={<Avatar alt={`${user.firstName} ${user.lastName}`}>{user.firstName?.[0] || '?'}</Avatar>}
              ...
              <ListItem
                ...
                secondaryAction={
                  selectedUsers.some(u => u.id === user.id) ? (
                    <IconButton sx={{marginRight: 1}} edge="end" onClick={() => handleRemoveUser(user.id)} color="error">
                      <PersonRemoveIcon />
                    </IconButton>
                  ) : (
                    <IconButton sx={{marginRight: 1}} edge="end" onClick={() => handleAddUser(user)} color="primary">
                      <PersonAddIcon />
                    </IconButton>
                  )
                }
                sx={{
                  ...
                  bgcolor: selectedUsers.some(u => u.id === user.id) ? 'rgba(25, 118, 210, 0.08)' : 'transparent'
                }}
              >
                <ListItemAvatar>
